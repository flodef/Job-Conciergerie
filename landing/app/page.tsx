...
function ThemeToggle({ mode, set, size = 'sm' }: { mode: ThemeMode; set: (m: ThemeMode) => void; size?: 'sm' | 'md' }) {
  ...
  return (
    <div className={`inline-grid grid-cols-3 gap-0.5 rounded-full ${padding} text-slate-400`} style={{ backgroundColor: 'var(--glass-bg)', border: '1px solid var(--glass-border)' }}>
      <button
        type="button"
        onClick={() => set('system')}
        title="Système"
        ...
        className={`rounded-full flex items-center justify-center transition-all ${size === 'sm' ? 'p-1.5' : 'p-2'} ${mode === 'system' ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white shadow-sm' : 'hover:text-white'}`}
      >
      ...
      {options.map(opt => (
        <button
          ...
          role="radio"
          style={{ cursor: 'pointer' }}
          className={`rounded-full flex items-center justify-center transition-all ${size === 'sm' ? 'p-1.5' : 'p-2'} ${mode === opt.value ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white shadow-sm' : 'hover:text-white'}`}
        >
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          {links.map(l => (
            <a key={l.href} href={l.href} onClick={() => l.subject && window.dispatchEvent(new CustomEvent('contactSubject', { detail: l.subject }))} className="text-sm text-slate-300 hover:text-white transition-colors">
              {l.label}
            </a>
          ))}
          <a href="#contact" onClick={() => window.dispatchEvent(new CustomEvent('contactSubject', { detail: 'demo' }))} className="px-5 py-2 rounded-full bg-gradient-to-r from-brand-500 to-accent-500 text-white text-sm font-semibold hover:opacity-90 transition-opacity">
            Demander une démo
          </a>
          <ThemeToggle mode={mode} set={set} size="sm" />
        </div>
        <div className="flex items-center gap-3">
          <button type="button" className="md:hidden text-slate-300 relative z-50" onClick={() => setMenuOpen(!menuOpen)} style={{ cursor: 'pointer' }}>
            {menuOpen ? <IconX size={24} /> : <IconMenu2 size={24} />}
          </button>
        </div>
      </div>
      {menuOpen && (
        <div className="md:hidden mt-3 mx-4 rounded-2xl p-6 flex flex-col gap-4" style={{ backgroundColor: 'var(--nav-bg)', border: '1px solid var(--glass-border)' }}>
          {links.map(l => (
            <a key={l.href} href={l.href} onClick={() => { setMenuOpen(false); l.subject && window.dispatchEvent(new CustomEvent('contactSubject', { detail: l.subject })); }} className="text-slate-300 hover:text-white transition-colors">
              {l.label}
            </a>
          ))}
          <a href="#contact" onClick={() => { setMenuOpen(false); window.dispatchEvent(new CustomEvent('contactSubject', { detail: 'demo' })); }} className="px-5 py-2 rounded-full bg-gradient-to-r from-brand-500 to-accent-500 text-white text-sm font-semibold text-center">
            Demander une démo
          </a>
...
function Hero() {
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-20 pb-16 overflow-hidden">
      <div className="hero-glow w-[600px] h-[600px] bg-brand-600/20 top-[-100px] left-[-100px] animate-glow" />
      <div className="hero-glow w-[500px] h-[500px] bg-accent-500/15 bottom-[-50px] right-[-50px] animate-glow" style={{ animationDelay: '1.5s' }} />
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        <p className="text-lg md:text-xl text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed">
          La plateforme qui connecte conciergeries et prestataires de confiance.
          Attribution automatique, suivi en temps réel, comptes rendus photo —
          tout réuni dans une application élégante et intuitive.
        </p>

        <div className="flex flex-col sm:flex-row gap-4 justify-center mb-16">
          <a href="#contact" onClick={() => window.dispatchEvent(new CustomEvent('contactSubject', { detail: 'demo' }))} className="px-8 py-4 rounded-full bg-gradient-to-r from-brand-500 to-accent-500 text-white font-semibold text-lg hover:scale-105 transition-transform shadow-lg shadow-brand-500/25">
            Demander une démo gratuite
          </a>
          <a href="#fonctionnalites" className="px-8 py-4 rounded-full glass glass-hover text-white font-semibold text-lg">
          ...
          {features.map((f, i) => (
            <div key={i} className="glass glass-hover glow-border rounded-2xl p-6 group">
              <div className="flex items-center gap-3 mb-3">
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center group-hover:scale-110 transition-transform flex-shrink-0`}>
...
function HowItWorks() {
  ...
  return (
    <section id="comment-ca-marche" className="relative py-16 md:py-24 px-6 overflow-hidden">
      <div className="hero-glow w-[400px] h-[400px] bg-brand-600/10 top-1/2 left-1/4" />
        ...
        <div className="space-y-6">
          {steps.map((s, i) => (
            <div key={i} className="flex flex-row gap-4 items-start">
              <div className="flex-shrink-0 w-14 h-14 rounded-2xl glass glow-border flex items-center justify-center">
...
function PhoneMockup() {
  ...
  const screenOrder: MockScreen[] = mode === 'employee'
  ...
  return (
    <section className="relative py-16 md:py-24 px-6 overflow-hidden">
      <div className="hero-glow w-[500px] h-[500px] bg-accent-500/10 top-1/2 right-0" />
      <div className="max-w-6xl mx-auto relative z-10 grid md:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p className="text-slate-400 text-lg leading-relaxed mb-8">
            Conçue pour le terrain. Vos prestataires ouvrent l'app sur leur téléphone,
            voient les missions disponibles, acceptent en un tap, démarrent leur journée
            et envoient le compte rendu photo à la fin. Le tout sans installation :
            ça fonctionne directement dans le navigateur.
          </p>
          <div className="space-y-3">
            {[
              ...
              <div key={i} className="flex items-center gap-3">
                <IconCircleCheck size={20} className="text-accent-400 flex-shrink-0" />
        ...
        <div className="flex flex-col items-center gap-6 w-full">
          ...
          <div className="relative animate-float origin-top">
            <div className="w-[280px] sm:w-[300px] h-[560px] sm:h-[600px] rounded-[2.5rem] glass border-2 border-white/10 p-3 sm:p-4 shadow-2xl">
              <div className="w-full h-full rounded-[2rem] bg-gradient-to-b from-slate-900 to-slate-950 overflow-hidden flex flex-col" style={{ background: 'linear-gradient(to bottom, var(--phone-bg-from), var(--phone-bg-to))' }}>
                ...
                <div className="flex justify-between items-center text-xs text-slate-500 px-4 pt-3 pb-2">
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${mode === 'employee' ? 'bg-brand-500/20 text-brand-300' : 'bg-accent-500/20 text-accent-300'}`}>
                ...
                <div className="flex-1 overflow-hidden px-3 pb-2">
                    ...
                    <div className="h-full flex flex-col gap-2.5">
                      <div className="flex items-center justify-between px-1">
                        ...
                        <div className="flex gap-1">
                          {[0, 1, 2, 3].map(s => (
                            <div key={s} className={`h-1.5 rounded-full transition-all ${s === missionStage ? 'w-4 bg-brand-400' : 'w-1.5 bg-slate-700'}`} />
                          ))}
                        </div>
                      </div>

                      <div className={`glass rounded-xl p-3 border transition-all duration-500 ${
                        missionStage === 0 ? 'border-amber-500/40 animate-pulse' :
                        missionStage === 1 ? 'border-brand-500/40' :
                        missionStage === 2 ? 'border-purple-500/40' :
                        ...
                        <div className="flex gap-2 mb-2">
                          <span className="text-xs px-2 py-0.5 rounded-full bg-brand-500/20 text-brand-300">Nettoyage</span>
                          <span className="text-xs px-2 py-0.5 rounded-full bg-accent-500/20 text-accent-300">Jardinage</span>
                        </div>
                        {mode === 'employee' && missionStage === 0 && (
                          <button type="button" className="w-full py-1.5 rounded-lg bg-gradient-to-r from-brand-500 to-accent-500 text-white text-xs font-semibold">
                            ✓ Accepter la mission
                          </button>
                        )}
                      ...
                      {missionStage === 0 && (
                        <div className="glass rounded-lg p-2 border border-amber-500/30 flex items-center gap-2 animate-float">
                          ...
                          <span className="text-xs text-amber-300">
                            {mode === 'employee' ? 'Nouvelle mission disponible !' : 'Mission en attente de prestataire'}
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="h-full flex flex-col gap-2">
                      ...
                      <div className="grid grid-cols-7 gap-1 text-center">
                        {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((d, i) => (
                          <div key={i} className="text-xs text-slate-600">{d}</div>
                        ))}
                        {Array.from({ length: 31 }, (_, i) => {
                          ...
                          const isToday = day === 5;
                          return (
                            <div key={i} className={`text-xs py-1.5 rounded-lg ${isToday ? 'bg-brand-500 text-white font-bold' : hasMission ? 'bg-brand-500/20 text-brand-300' : 'text-slate-500'}`}>
                              {day}
                            </div>
                          );
                        })}
                      ...
                      <div className="mt-2 space-y-1.5">
                        ...
                        <div className="glass rounded-lg p-2 flex items-center gap-2">
                          ...
                          {mode === 'conciergerie' && <span className="ml-auto text-xs text-slate-500">Non assignée</span>}
                        </div>
                        <div className="glass rounded-lg p-2 flex items-center gap-2">
                      ...
                      <div className="glass rounded-xl p-3">
                        ...
                        <div className="flex items-end justify-around h-14 gap-2">
                          {[40, 65, 50, 80, 35].map((h, i) => (
                            <div key={i} className="flex-1 rounded-t bg-gradient-to-t from-brand-600 to-brand-400 transition-all duration-700" style={{ height: `${h}%` }} />
                          ))}
                        </div>
                        <div className="flex justify-around mt-1">
                          {['S1', 'S2', 'S3', 'S4', 'S5'].map((w, i) => (
                            <span key={i} className="text-xs text-slate-600">{w}</span>
                          ))}
                        </div>
                      </div>
                  ...
                  {screen === 'biens' && (
                    <div className="h-full flex flex-col gap-2">
                      ...
                      {[
                        ...
                        <div key={i} className={`glass rounded-xl p-2.5 ${b.status === 'En pause' ? 'opacity-50' : ''}`}>
                          <div className="flex items-center gap-2 mb-1">
                            <IconHome size={14} className="text-brand-400 flex-shrink-0" />
                            <span className="text-sm font-semibold text-white flex-1 truncate">{b.name}</span>
                            <span className={`text-[10px] px-2 py-0.5 rounded-full ${b.status === 'Actif' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-500/20 text-slate-500'}`}>
                              {b.status}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-xs text-slate-500">
                  ...
                  {screen === 'prestataires' && (
                    <div className="h-full flex flex-col gap-2">
                      ...
                      {[
                        { name: 'Karim B.', initials: 'K', missions: 12, status: 'Disponible', color: 'from-brand-500 to-accent-500' },
                        { name: 'Sophie L.', initials: 'S', missions: 8, status: 'En mission', color: 'from-purple-500 to-pink-500' },
                        { name: 'Marc D.', initials: 'M', missions: 5, status: 'Disponible', color: 'from-cyan-500 to-blue-500' },
                        { name: 'Élodie M.', initials: 'É', missions: 3, status: 'Indisponible', color: 'from-amber-500 to-orange-500' },
                      ].map((p, i) => (
                        <div key={i} className="glass rounded-xl p-2.5 flex items-center gap-2.5">
                          <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${p.color} flex items-center justify-center text-white font-bold text-xs flex-shrink-0`}>
                            {p.initials}
                          </div>
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full flex-shrink-0 ${
                            p.status === 'Disponible' ? 'bg-emerald-500/20 text-emerald-400' :
                            p.status === 'En mission' ? 'bg-purple-500/20 text-purple-400' :
                  ...
                  {screen === 'calendar-conciergerie' && (
                    <div className="h-full flex flex-col gap-2">
                      ...
                      {[
                        { name: 'Karim B.', initials: 'K', color: 'from-brand-500 to-accent-500', missions: [
                          { title: 'Villa Saint-Cloud', time: '09:00', status: 'Terminée', color: 'text-emerald-400' },
                        ...
                        { name: 'Sophie L.', initials: 'S', color: 'from-purple-500 to-pink-500', missions: [
                        ...
                        { name: 'Marc D.', initials: 'M', color: 'from-cyan-500 to-blue-500', missions: [
                          { title: 'Appart Marais', time: '08:00', status: 'Terminée', color: 'text-emerald-400' },
                        ] },
                      ].map((p, i) => (
                        <div key={i} className="glass rounded-xl p-2.5">
                          <div className="flex items-center gap-2 mb-1.5">
                            <div className={`w-6 h-6 rounded-full bg-gradient-to-br ${p.color} flex items-center justify-center text-white font-bold text-[10px] flex-shrink-0`}>
                              {p.initials}
                            </div>
                  ...
                  {screen === 'settings' && (
                    <div className="h-full flex flex-col gap-2">
                      ...
                      {[
                        ...
                        { icon: IconMail, label: 'Email', value: mode === 'employee' ? 'karim@mail.fr' : 'sophie@mail.fr' },
          ...
          <div className="inline-flex items-center gap-2 rounded-full p-1.5" style={{ position: 'relative', zIndex: 50, backgroundColor: 'var(--glass-bg)', border: '1px solid var(--glass-border)' }}>
            <button
              type="button"
              onClick={() => setMode('employee')}
              style={{ cursor: 'pointer' }}
              className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${mode === 'employee' ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              👤 Prestataire
            </button>
            <button
              ...
              style={{ cursor: 'pointer' }}
              className={`px-5 py-2 rounded-full text-sm font-semibold transition-all ${mode === 'conciergerie' ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              🏢 Conciergerie
            </button>
          </div>
          ...
          {testimonials.map((t, i) => (
            <div key={i} className="glass glass-hover rounded-2xl p-6">
              <div className="flex gap-1 mb-4">
                ...
                {t.stars % 1 !== 0 && (
                  <IconStarHalf size={16} className="fill-amber-500 text-amber-500" />
                )}
              </div>
...
function Pricing() {
  ...
  const tooltipTexts: Record<string, string> = {
    ...
    'Personnalisation avancée': 'Adaptation de l\'application à vos besoins spécifiques (workflows, champs personnalisés)',
    ...
    <section id="tarifs" className="relative py-16 md:py-24 px-6 overflow-hidden">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-12">
          ...
          <div className="inline-flex items-center gap-4 rounded-full p-1.5" style={{ position: 'relative', zIndex: 50, backgroundColor: 'var(--glass-bg)', border: '1px solid var(--glass-border)' }}>
            <button
              type="button"
              onClick={() => setAnnual(false)}
              style={{ cursor: 'pointer' }}
              className={`px-6 py-2 rounded-full text-sm font-semibold transition-all ${!annual ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Mensuel
            </button>
            <button
              ...
              style={{ cursor: 'pointer' }}
              className={`px-6 py-2 rounded-full text-sm font-semibold transition-all flex items-center gap-2 ${annual ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white' : 'text-slate-400 hover:text-white'}`}
            >
              Annuel
              <span className={`text-xs px-2 py-0.5 rounded-full ${annual ? 'bg-white/20 text-white' : 'bg-accent-500/20 text-accent-300'}`}>
                2 mois offerts
              </span>
            </button>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((p, i) => (
            <div
              key={i}
              className={`relative z-10 rounded-2xl p-8 flex flex-col ${p.highlighted ? 'glass glow-border border-2 border-brand-500/30' : 'glass'}`}
            >
              {p.badge && (
              ...
              <div className="space-y-3 mb-8 flex-1">
                {p.features.map((f, j) => {
                  ...
                  return (
                    <div key={j} className="flex items-center gap-2">
                      <IconCheck size={18} className="text-accent-400 flex-shrink-0" />
                      {tooltip ? (
                        <span className="text-sm text-slate-300 flex items-center gap-1">
                          ...
                          >
                            <button type="button" onClick={(e) => { e.stopPropagation(); setOpenTooltip(openTooltip === f ? null : f); }} style={{ cursor: 'pointer' }}>
                              <IconSparkles size={14} className="text-slate-500 hover:text-accent-400" />
                            </button>
                            {(openTooltip === f) && (
                              <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 rounded-lg text-xs text-white max-w-[200px] w-max z-50 pointer-events-none" style={{ backgroundColor: 'var(--bg-base)', border: '1px solid var(--glass-border)', opacity: 1 }} onClick={() => setOpenTooltip(null)}>
                                {tooltip}
                              </span>
                            )}
                          </span>
              ...
              <a
                ...
                className={`block text-center py-3 rounded-full font-semibold transition-all relative z-20 ${
                  p.highlighted
                    ? 'bg-gradient-to-r from-brand-500 to-accent-500 text-white hover:scale-105'
...
function ContactForm() {
  ...
  const inputClass = 'w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-slate-600 focus:outline-none focus:border-brand-500/50 focus:bg-white/8 transition-all';
  const selectClass = inputClass + ' appearance-none cursor-pointer';

  return (
    <section id="contact" className="relative pt-16 md:pt-24 pb-8 md:pb-12 px-6 overflow-hidden">
      <div className="hero-glow w-[500px] h-[500px] bg-brand-600/15 top-0 left-0" />
      <div className="hero-glow w-[400px] h-[400px] bg-accent-500/10 bottom-0 right-0" />
      <div className="max-w-5xl mx-auto relative z-10">
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-black mb-4">
          ...
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Voyons ensemble comment Job Conciergerie peut transformer votre activité.
        ...
        <div className="grid md:grid-cols-5 gap-8">
          ...
          <div className="md:col-span-3 md:order-1">
            <form onSubmit={handleSubmit} className="glass rounded-2xl p-8 space-y-4">
              ...
              <div>
                ...
                >
                  <option value="" style={{ color: 'var(--text-muted)' }}>Choisir un sujet...</option>
                  <option value="forfait-decouverte" style={{ color: 'var(--text-primary)' }}>Forfait Découverte</option>
                  <option value="forfait-pro" style={{ color: 'var(--text-primary)' }}>Forfait Pro</option>
                  <option value="forfait-privilege" style={{ color: 'var(--text-primary)' }}>Forfait Privilège</option>
                  <option value="demande-renseignement" style={{ color: 'var(--text-primary)' }}>Demande de renseignement</option>
                  <option value="demo" style={{ color: 'var(--text-primary)' }}>Demande de démo</option>
                </select>
              </div>
              <div>
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl font-semibold text-lg flex items-center justify-center gap-2 transition-all ${
                  status === 'sent'
                    ...
                    : 'bg-gradient-to-r from-brand-500 to-accent-500 text-white hover:scale-[1.02]'
                }`}
              >
                {status === 'sending' && (
                  <>
          ...
          <div className="md:col-span-2 md:order-2 space-y-6">
            <div className="glass rounded-2xl p-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={i} className="flex items-center gap-3">
                    <item.icon size={20} className="text-accent-400 flex-shrink-0" />
            ...
            <div className="glass rounded-2xl p-6 space-y-4">
              <a href="mailto:contact@job-conciergerie.fr" className="flex items-center gap-3 text-slate-300 hover:text-white transition-colors">
                <IconMail size={20} className="text-brand-400" />
                <span className="text-sm">contact@job-conciergerie.fr</span>
              </a>
              <a href="tel:+33620718834" className="flex items-center gap-3 text-slate-300 hover:text-white transition-colors">
              ...
              <div className="flex items-center gap-3 text-slate-300">
                ...
                <span className="text-sm">Brest, Finistère</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-6 text-sm text-slate-500">
            ...
            <a href="#contact" onClick={() => window.dispatchEvent(new CustomEvent('contactSubject', { detail: 'demande-renseignement' }))} className="hover:text-white transition-colors">Contact</a>
          </div>
