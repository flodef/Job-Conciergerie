...
export default function CheckoutContent() {
  ...
  const [orderMode, setOrderMode] = useState<'sandbox' | 'prod'>('sandbox');
  ...
  useEffect(() => {
    ...
    const createOrder = async () => {
      ...
      try {
        ...
        if (res.status === 503) {
          window.location.href = `/?subject=forfait-${plan}#contact`;
          return;
        }
        ...
        if (mode === 'prod' || mode === 'sandbox') setOrderMode(mode);
        setOrderToken(token);
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const mountWidget = async () => {
      try {
        ...
        const instance = await RevolutCheckout(orderToken, orderMode);
    ...
    return (
      ...
      >
        <div className="glass rounded-2xl p-8 max-w-md text-center">
          ...
          <a
            href="/"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-linear-to-r from-brand-500 to-accent-500 text-white font-semibold hover:opacity-90 transition-opacity"
          >
