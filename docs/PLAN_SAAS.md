...
- Reset à la demande : `GET /api/demo/enter` (public) — le lien « Essayer la démo » de la landing y mène : wipe + reseed (cadence min 60 s, sérialisé par `pg_advisory_lock`) puis redirect 303 vers `/<demo_id>`. Accès direct à `demo.*` → pas de reset. Reset admin : `GET/POST /api/demo/reset` (`Bearer $CRON_SECRET` ou `?key=$DEMO_RESET_KEY`, lazy >12h sauf `?force=1`) et `bun scripts/seed-demo.ts`.
